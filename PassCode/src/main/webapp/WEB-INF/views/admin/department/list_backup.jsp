    ...
    <div class="card-custom mb-4">
        ...
        <div class="card-body-custom">
            <form method="get" action="${pageContext.request.contextPath}/admin/department/list">
                <div class="row">
                    <div class="col-md-4">
                        <label class="form-label dark-label">部门名称</label>
                    ...
                    <div class="col-md-4">
                        <label class="form-label dark-label">部门代码</label>
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div class="result-info">
                <span class="text-light-muted">
                    共找到 <strong class="text-primary">${departments.size()}</strong> 个部门
                </span>
            </div>
            <div class="batch-actions">
                <button type="button" class="btn-gradient-secondary" onclick="exportDepartments()">
                    <i class="bi bi-download me-2"></i>导出部门
                </button>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="department-info">
                                                ...
                                                <c:if test="${not empty department.parentDepartment}">
                                                    <div class="text-light-muted small">
                                                        <i class="bi bi-arrow-up-right me-1"></i>
                                                        隶属于: ${department.parentDepartment}
                                                    </div>
                                                </c:if>
                                            </div>
                                        </td>
...
function viewDepartment(departmentId) {
    fetch('${pageContext.request.contextPath}/admin/department/detail/' + departmentId)
                        ...
                        '<div class="col-md-6">' +
                            '<div class="detail-item">' +
                                ...
                                '<div class="detail-value">' +
                                    '<span class="badge-' + (department.status == 'ACTIVE' ? 'success' : 'secondary') + '">' +
                            ...
                            '<div class="detail-item">' +
                                ...
                                '<div class="detail-value">' + (department.createTime || '未知') + '</div>' +
                            '</div>' +
                            '<div class="detail-item">' +
                                '<label class="detail-label">人员数量</label>' +
                                '<div class="detail-value">' + (department.memberCount || 0) + ' 人</div>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
                    (department.description ? 
                        '<div class="detail-item mt-3">' +
                            '<label class="detail-label">部门描述</label>' +
...
function toggleDepartmentStatus(departmentId, currentStatus) {
    ...
    if (confirm('确定要' + actionText + '这个部门吗？')) {
        fetch('${pageContext.request.contextPath}/admin/department/toggle-status/' + departmentId, {
            method: 'POST',
            headers: {
...
function deleteDepartment(departmentId) {
    if (confirm('确定要删除这个部门吗？此操作不可撤销，该部门下的所有预约记录也将受到影响。')) {
        fetch('${pageContext.request.contextPath}/admin/department/delete/' + departmentId, {
            method: 'DELETE'
        })
        .then(response => response.json())
        .then(data => {
...
function exportDepartments() {
    window.location.href = '${pageContext.request.contextPath}/admin/department/export?format=excel';
}
